...
from binary.module import Import, ImportDesc, ImportTagFunc, ImportTagTable, ImportTagMem, ImportTagGlobal, Global, \
    Export, ExportDesc, ExportTagFunc, ExportTagTable, ExportTagMem, ExportTagGlobal, Elem, Code, Locals, Data, \
    MagicNumber, Version, Module, SecCustomID, SecDataID, CustomSec, SecTypeID, SecImportID, SecFuncID, SecTableID, \
    SecMemID, SecGlobalID, SecExportID, SecStartID, SecElemID, SecCodeID
...
class WasmReader:
    ...
    def read_sections(self, module: Module):
        ...
        while self.remaining() > 0:
            ...
            if sec_id == SecCustomID:
                module.custom_secs = self.read_custom_sec()
                continue
            ...
            self.read_non_customsec(sec_id, module)
    ...
    def read_custom_sec(self):
        ...
        return CustomSec(name=sec_reader.read_name(), bytes=sec_reader.data)

    def read_non_customsec(self, sec_id, module):
        if sec_id == SecTypeID:
            module.type_sec = self.read_type_sec()
        elif sec_id == SecImportID:
            module.import_sec = self.read_import_sec()
        elif sec_id == SecFuncID:
            module.func_sec = self.read_func_type()
    ...
    def read_code(self):
        ...
        code = Code(locals=code_reader.read_locals_vec())
    ...
    def read_func_type(self):
        ft = FuncType(self.read_byte(), self.read_val_type(), self.read_val_type())
        if ft.tag != FtTag:
